...
AI에게 프로젝트 전체를 그대로 넘기는 방식은 확장성이 없습니다 — 설령 토큰 한도 안에 다 들어가더라도, 토큰이 많다고 컨텍스트가 좋아지는 건 아닙니다. Ziplex는 다음 네 가지 구체적인 실패 양상을 해결하기 위해 만들어졌습니다:

| 문제 | 원인 | Ziplex의 해결 방식 |
|---|---|---|
| **컨텍스트 오버플로우** | 원본 프로젝트가 모델의 토큰 한도를 초과 | Tree-sitter로 함수 본문을 제거하고, `tiktoken`으로 여러 모델 기준 결과를 측정 |
| **Lost in the middle** | 길고 평평한 컨텍스트 속에 중요한 내용이 묻힘 | 텍스트 덩어리 대신 파일별로 구조화된 JSON |
| **주의 분산 (Attention dilution)** | 불필요한 토큰이 정작 중요한 내용을 가림 | 기본적으로 모든 파일을 한 줄 요약으로 축소 |
| **의미적 공백 (Semantic gap)** | 파일을 그냥 나열해서는 서로 어떻게 연결되는지 안 보임 | 실제 import와 사람의 검토를 거쳐 만든 명시적 의존성 그래프(`relationships`) |
